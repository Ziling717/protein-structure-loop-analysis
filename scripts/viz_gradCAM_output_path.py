...
def draw_cam(window, cam, pdb_chain, goterm, sequence, out_dir):
    ...
    # plt.savefig(os.path.join(args.out_dir, "saliency_fig_" + pdb_chain + "_" + goterm.replace(':', '.') + ".png"))
    plt.savefig(os.path.join(out_dir, f"saliency_fig_{pdb_chain}_{goterm.replace(':', '.')}.png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default='.', help='Directory to save output files.')
    ...
    if args.list_all:
        ...
        viz_filename = os.path.join(args.out_dir, f"pymol_viz_{chain_id}_{go_id.replace(':', '_')}.py")
        with open(viz_filename, 'w') as f:
        # with open(os.path.join(args.out_dir, 'pymol_viz.py'), 'w') as f:
            ...
            for c in range(0, len(lchains)):
                ...
                draw_cam(window, np.array(new_cam), lchains[c], lgoterm[c], lseq[c], args.out_dir)
